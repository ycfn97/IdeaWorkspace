 ...
 * Package:
 * ClassName: SparkStreaming07_window
 *
 * @author 18729 created on date: 2020/10/31 15:52
 * @version 1.0
 * @since JDK 1.8
 */
object SparkStreaming07_window {
  def main(args: Array[String]): Unit = {
    ...
    value.flatMap(_.split(" ")).map((_,1)).window(Seconds(12), Seconds(3)).reduceByKey(_+_).print()
    //启动采集器
    ssc.start()
    //默认情况下，上下文对象不能关闭
    //ssc.stop()
